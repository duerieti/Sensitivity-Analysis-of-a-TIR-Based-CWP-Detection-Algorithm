...
# Set GDAL environment before any I/O
Sys.setenv(GDAL_CACHEMAX   = "8000")
Sys.setenv(GDAL_SWATH_SIZE = "4294967296")  # 4GB swath for sequential ops
...
# tiling + compression creation options
# ZSTD: best balance of speed and ratio for HPC network filesystems
# PREDICTOR=2 improves compression ratio for continuous integer/float data
co_int   <- "--co TILED=YES --co BLOCKXSIZE=512 --co BLOCKYSIZE=512 --co BIGTIFF=YES --co COMPRESS=ZSTD --co PREDICTOR=2"
co_float <- "--co TILED=YES --co BLOCKXSIZE=512 --co BLOCKYSIZE=512 --co BIGTIFF=YES --co COMPRESS=ZSTD --co PREDICTOR=3"
# PREDICTOR=2 for integers (zone_r_big)
# PREDICTOR=3 for floats (r_rounded, Tmean_raster, binary_out)
# floating point predictor encodes differences between adjacent values,
# which works better than integer differencing for temperature data
...
# zone_r_big: integer data → PREDICTOR=2
system(paste0(
...
# r_rounded: float data → PREDICTOR=3
...
# Tmean_raster: float data → PREDICTOR=3
tic()
system(paste0(
...
# binary_out: contains 1 and NaN only — highly compressible, float → PREDICTOR=3
tic()
system(paste0(
  'gdal raster calc ',
  '-i "A=r_rounded.tiff" ',
