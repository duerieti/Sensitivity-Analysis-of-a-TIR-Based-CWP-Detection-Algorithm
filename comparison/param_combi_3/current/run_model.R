...
library(tictoc)    # max cells per dissolve chunktictoc) # for timing
...
patches_new <- detect_cwp_single(
    ...
    step_m            = 1200,       # slab length along centerline (m)
    slab_halfwidth_m  = 60,
    buffer_px         = 5,         # corridor half-width for reference T (pixels)
    delta_C           = 1.5,       # temperature anomaly threshold (deg C)
