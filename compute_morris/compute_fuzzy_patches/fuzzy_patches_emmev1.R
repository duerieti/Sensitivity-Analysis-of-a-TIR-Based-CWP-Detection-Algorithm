...
dir.create("./binary_rasters", showWarnings = FALSE)

for (i in seq_along(read_paths)) {
  out_path <- file.path("./binary_rasters", paste0("rast_", i, ".tif"))
  system(paste0(
...
system(
  paste0(
    'gdal raster calc -i "A=stacked_raster.vrt" --flatten --calc "avg(A)" -o fuzzy_cwp.tif'
  )
