...
#### Article covering exact_extract and terra::extract

Some tests seem to underline that exact_extract is slower than terra::extract for small rasters but much faster for large rasters. Also extracting on mutlilayer rasters is faster than on single rasters. The second statement of course makes rather a lot of sense. If a raster has the same extent and resolution, then we only need to solve the problem for one pixel. The corresponding pixels in all other layers will then of course also be in the polygon bounds. => This can be used to optimize further. This, because I have to do two extraction steps at the end. I can combine them together.

https://tmieno2.github.io/R-as-GIS-for-Economists/extract-speed.html

Its a book and it was written by a Lecturer. So i beliefe it is niece but citation worthy. 


#### Is terra::extract vs. exact_extract the last hurdle explaining the differences

To test this, i got to run an adapted version of my function that uses terra::extract on the hpc and see. I hope it is, because this would then finaly reviel the differences.


#### Exact timing and proviling

I need to time and profile the two functions properly. Need to find a profiler that shows how much time is spent in compiled C routines and less time is spent in native R. It would then be great to see how this time is beeing reduced in my function.
...
I guess this time it would be very important to first clearly decide what to communicate, then think about great visualisations of my results and findings. Perception is humans strongest sense to process data and analyse patterns. Also I need to demonstrate, that I improved in using visualisations. I would recommend quarto to do this, as this shows some extra care and looks more proper. At the same time it is very text based which is an ideal format for applying some AI as an aid.


#### Found last issue

I tracked down the last issue I guess. The problem was located in the mean computation. terra::extract includes all pixels which center lies within the polygon. exact_extract also includes snipets of polygons and just weights them by their coverage. This allone has lead to a large enough difference in the computed median, that it had an effect on pixel flagging and also rejudging polygons. Luckely there is a way to tric around and write a median function that only includes pixels with a higher coverage than 0.5. This mimics the behaviour of terra::extract while using exact_extract which is much much much faster.


### Now onto profiling

Now the improved script needs to be properly encapsulated into a function, and then of to profiling on the HPC.

Not quite certain wheter profiling is even of use here. System calls are not recorded in time so its hard to determine.

### Comparison

To compare, wheter both functions truly yield the same results, I am trying 3 different parameter combinations and then I do some visual comparison. It has to be noted that this is of course not realy a rigorous way of testing this. But I can not do a mathematical proof or something. So this is just the best I got currently.


I checked two parameter combinations. The results are exactly the same down to the very pixel. Here are the parameter combinations I tried:
...
It would be interesting to now how much memory the new function and the old function need to get by. If the new function needs significantly less memory, then this would be another great argument for the funciton. Because parallelizing on multiple cpus is also only realy possible, if not all the memory of a node is exausted just by running a view parallel processes. If 10 processes allready use up the 200 GB of memory, then only 10 out of 20 cores realy can be used. So in a way memory is connected to time over resource efficiency.

I asume that the new function should get by with only 5 gigabites of memory. Not quite shure about the old function. Pretty shure terra::extract will cause a crash here.

new function can not get by using only 5 GB of memory. Lets hope that 9 GB wors. Because 20 * 9 = 180 GB which would still fit onto one node. 

Success, the new method can run on 9 GB of memory. Nothing stands in the way of parallelisation anymore interms of compute and memory. What is still open is to check wheter there is ample networktraffic that would slow other users down and wheter the new function reproduces the old function also for different parameter combinations.

The old function crashed on 5 GB. Trying 25 now. 25 worked. Going down to 10 now (9.8 for terraOpts).


Something that i also just noticed or I am starting to expect, is that my function scales wayy wayy better with an increased number of slabs. Because the code that runs with 50 slabs is running for 3h allready. But the new function does not mind at all how meany slabs there are. It does not matter algorithmicly. Because all the heavy stuff is just a bunch of raster algebra. Does not realy matter what value a pixel has, what mean is computed per slab and how meany slabs there are etc. Every pixel just gets computed a fixed number of times. My version only takes 27 minutes for 50 meter slabs.
...
one shapefile packet (.shp, .prj, .dbf, etc) takes up approx 10 MB. A thousand of those will be 10 GB. We got 300 GB on scratch => We are good on that

One process needs 9 GB. A node has 200 GB and 20 cores. 20 * 9 GB = 180 GB -> We are good on that.


We can use GNU parallel to set up the parallelisation

Its an embarresingly parallel problem. No communication what so ever needed between workers. Workers can optain variables from a


### Pivoting to Morris indices

Sobol indices are the master class of sensitivity analysis. But they are difficult and very expensive to compute. The hypthotesis that we definded can also be answered using Morris indices. + Morris indices are much easier to understand (in my reach of understanding) and computionally much less expensive. So it is only reasonable to go for Morris instead. Because the idea is to have a metric of how much individual parameters affect the outcome spatially -> Morris is likely enough for that. The rest of a good project and therefore a good grade comes from things like lear comunication, good visualsiations, etc. So diving ultra deep into Sobol will not yield any benefit. But 

#### Sampling parameters in Morris

Its not that tremendously complicated. Exaclty the right thing for a theory secion in a BSc project. The libary `sensitivity` can be used to set up the grid. Then this can be saved to a textfile and read by gnu parallel.


Kind of hitting a wall since I am not able to load a specific package called sensitivity on the hpc. But I need this package to set up the parameter combinations. Ok it worked now luckely.



#### Disk management

I noticed that my function is writing huge rasters to disk. One raster is like 25GB. So If I run functions in parallel, this is a recepie for disaster: 25GB * 20  is allready above my disk limit. I need to compress and delete intermediary results.

zone_r_big_asc and zone_r_big_desc are only 1 GB, so deleting them as soon as they are no longer used suffices, no compression needed.
Tmean_raster_asc.tiff, Tmean_raster_desc.tiff and Tmean_raster.tiff are each 1.5 GB. So there is also no compression necessairy.
Binary_out.tif is 25 gb. so compression is definitely needed here, or something else (maybe writing byte datatype ?)
r_rounded.tiff is 50 GB, so this also needs to be addressed. => Since the rounding step is always the same, we could simply just set this as the new entry raster into the algorithm -> So only compute it once => disk save + time save


binary_out.tif currently stores Float32 numbers just to represent 0 and 1. This is unnecessairly large datatype. I set it to byte now. Theoretically this should allready decrease size by a factor of 4. But we might still have to compress it. It came down to 4.5 G now. => so roughly a factor of 5

2 x 1.0 + 3 x 1.5 + 4.5 = 2.0 + 4.5 + 4.5 = 11 GB

11 GB * 20 = 220 GB


But some rasters allready get deleted before others get created. From that perspective this is most likely aready enough. But for good measure I deployed compression on binary_out.tif. Its now only 4.4 MB. So we are around 6.5 GB now. This is low enough.

Also i stored r_rounded.tiff in data and the function now just gets it from there => Another 50 GB eliminated!! 



### Something fishy is happening at the last exact_extract.

Somehow I got an Out of memory error. Weird memory spike I guess.



### GNU Parallel, another try

With the disk quota beeing the likely culprit in the last run, lets try GNU parallel with this improved function. around 300 runs, then we are allready finished and ready to answer the hypothesis


### Improved sampling

I read satelli et all to realy get the nack of morris and understand how the sampling method works on a deeper level. I think it is a very intuitive yet powerfull method and it is much much cheaper than sobol.

I also picked sensible number of levels and delta accoring to satelli

Sobol would require 800 model runs and also I am rather certain, that the we would run into issues as sobol requires continuous sampling ? Not quite certain about that one. But I gues the 800 runs are just very very expensive. And in the end I think a project work also needs to be written along the following rational: The main goal is to hand in a report with 10 000 words that covers all the necessairy chapters. It does not serve anybody if one of these chapters presents a perfectly exectuted and difficult method if the rest is trash due to time constraints.

So better just take morris and lump the output into m² area and temperature and stuff like this. Because this is easily understandable, this will work with a high likelyhood, one can produce clear and easy plots and discussion is managable. So it is just the better overall package. And also i only need 130 model runs. So plenty of time to rerun everything in case something does not work out.


### Selection of tributary areas

I scanned though the river section and selected suited areas for the analysis.

The first one is the Limpbach. This was recommended by Diego and Manuel.

The second one I picked myself. For both I realy should check wheter the tributaries feature a significantly different temperature value. => This is also part of the hypothesis.


I also searched for areas that feature coldspots but no tributary rivers in sight. These seem to be surprisingly sparse. I found one near hasle bei Burgdorf. I found another one near emmepark right after bätterkinden.


Now we can track temperature and size of these 4 areas and use them as a lumped measure in morris.

### Lumping

The job on the HPC ran succesfully. All 133 parameter combinations were computed. I now ziped them together and downloaded them using scp. Then I did the spatial lumping in a script the result is a csv file holding the resulting measures of all 4 locations for every parameter combination. Before I do the morris, I think it would be whise to check wheter the grüene tributary and the limpbach tributary are indeed colder than the emme. A quick visual verification should suffice here. But I got to operationalise this decision later on. What does "significantly different temperature" mean? This of course needs to be defined. But for now, just having a look at it in QGIS should be sufficient.


#### Are the tributaries significantly colder?

I am rather certain that it holds for the urtene but the grüene I think is not significantly different in temperatue. So we have to pick another tirbutary

I actually checked their report and found that the "Grundbach" also causes coldwater patches. So lets just replace the Grüene with the grundback.

Then the Limpbach also is a confirmed cold water tributary.

#### The first results

The first results look very promissing. I had to normalize the moris indices, which I did not read in the literature but claude code provided this as a solution. So I will definitely have to check this out in the summary. But the normalisaiton sounds rather sound. And the first results realy seem to confirm the hypothesis ( of course it would be best to perform the analysis for more segments to increase statistical robustness and also maybe increase r). Emmepark and hasle show very small variability against both the slab width aswell as the buffer_width.

Limpbach and grundbach on the other side show tremendous influence with regards to the lab with. On average one mean for grundbach and 1.42 for limpbach. Additionally both limpbach and grundbach also feature a large sigma, indicating that there is either nonlinearity present or interaction effects.


#### A great way to visualize

The humans most capable sense for interpretation is vision, so naturally it would best to also provide visual evidence/explenation. Maybe staking a bunch of polygons with different alphas. This would allow the reader to see how dramatically the patches vary and therefore it would make the large values of morris both more beliefable and also more understandable.

#### How to continue

-> Run variance decomposition for emme segment 2 to get more tributary influenced and non influenced areas so that statistical robustnes increases.


### Running on emme v2

I figured out why running on emme v2 does not work. the raster of emmev2 is a bit shorter than the path? No there is somehting wrong with the slab generation I am rather certain. Because one slab at the end is just simply missing. No that is not true, the collor was just white

The pixel size and the resolution where not perfectly aligned => THis was the issue. So i fixed this using GDAL on the command line. Used gdal raster repoject and gdal raster edit for that


### Running on obere emme 2

Now I will run the algo on obere emme v2. Here I had issues that rounding the raster would fail on the hpc. I now reexported through QGIS. And it worked on my windows pc. Simply reexporting did the trick. So lets uppload that to the HPC and then we are allready set to go. Maybe we could try to improve the number of cores here to push through 20 parallel jobs at once or even 30 parallel jobs. This way with two nodes we can deffo get a pretty decent troughput and maybe run an essemble with around 500 executions reasonably fast. From that we can then derive the fuzzy sets. These are then pretty much independent from any input parameter because **there is no input parameter**. They got averaged out trough the ensemble. 

Again the same issue. The input or the resolution or whatever does not seem to be propely alligned or something along the line of that. So I got to debug again it seems.

Yes gdal raster info shows that the origins do not perfectly allign and the pixel size is slightly different too. But with gdal raster reporject this is done quite neatly



# Section 2 - Addjustments to the algorithm

A good first approach would be to build on this multy layer averaging approach and just create a virtual raster that stacks multible versions of slab length and buffer width and then averages these out. What is funny is, that then of course the setting parameters dissapear. So how to measure then wheter this reduces variability => Key: We do not have to change anything at all then. Just compare to the fuzzy set output and determine from that wheter this is sensical. And when it produces senscial results for the whole emme, then the answer is: This version does not have any settings at all and works well. A bit of a sloppy approach howerver. But well what to do? Run a complete assemble would be another possibility and actually that is the more whise choice I assume. And the fuzzy mach can of course also be sold as a strength rather than an issue. + A huge chunk of the code can just be recycled and we can just draw random parameters from a uniform distribution or whatever


The results (of what I have seen) suggest that this effect of cutting of coldwaterpatches is realy just the major issue. the pixel with does not seem to be as challening. I therefore Am going to direct my fixes towards this region.

The following concept I came up with:

1. The deffinition of a coldwater patch states that a  coldwaterpatch is a area that is 1 Degree celsius colder than the "surrounding". Now to operationalize the idea of surrounding. This is realy what is tricky, because depending on what is beeing defined as the surrounding, you get different results. There are many "surroundings". Is it the mean of the current slab, or the mean of the next slab. Or the average? Or the mean of mutlible very small slabs. This is hard to say. I believe conceptually it could be interresting to add this diversity of the term "surrounding" into the algorithm. My approach is currently the following: 

1. sample random slab lengths from an uniform distirbution.
2. for every slab length:
  2.1 compute the mean temperatures for the slabs over the ruounden raster
  2.2 compute the centroid of each slab and assign the mean temperautre to the centroid
3. collect all the centorid points with the associated mean temperature. This results in a cluster of points scattered throughout the river. Each point encodes a measurement of the surrounding temperature at a specific location represented though the slab mean.
4. do some sampling to get an even density of points around the net (as good as possible)
5. compute a mean temperature raster using inverse distance weighting of the centroids to compute a mean environment temperature at every pixel location
6. Use this raster to flag coldwater pixel
... The rest is equivalent to the current version of the algorithm


In order to do this, some custom logic had to be implemented using Rcpp and parallelisation had to be used. this because the algorithm scales with O(m * n) (i believe) and m = 6 * 10^7 and n around 100. So the fastest code possible (compiled to get highly effective byte code) and parallelized to run in parallel is needed here to even make this feasible.

The only downside is that we add some new parameters to the algorithm but yea, I mean in the hand I need to be able to write 10'000 words of report and it is inherently difficult to derive an algorithm with no external assumptions that then express themselfs as hyperparameters.
