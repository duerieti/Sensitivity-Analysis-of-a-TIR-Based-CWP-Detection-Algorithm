...
# ── CHANGE 1: exact_extract(r_rounded, slaps_buffer_sf, fun = "median")
# terra::extract() needs a SpatVector; we summarise per polygon ID afterward.
tic()
raw_extract <- terra::extract(r_rounded, slaps_buffer_vect, fun = NULL, na.rm = TRUE)
slap_means  <- raw_extract %>%
  group_by(ID) %>%
  summarise(slap_means = median(T, na.rm = TRUE), .groups = "drop") %>%
  pull(slap_means)
toc()
...
# ── CHANGE 2: exact_extract(r, patches_large, c("mean","min","max","median"))
# terra::extract() with fun=NULL returns all cell values; we aggregate manually.
patches_large_vect <- terra::vect(patches_large)

tic()
raw_stats <- terra::extract(r, patches_large_vect, fun = NULL, na.rm = TRUE)
stats_matrix <- raw_stats %>%
  group_by(ID) %>%
  summarise(
...
# ── CHANGE 3: exact_extract(Tmean_raster, patches_large_w_stats, fun = "median")
patches_large_w_stats_vect <- terra::vect(patches_large_w_stats)

tic()
raw_slap <- terra::extract(Tmean_raster, patches_large_w_stats_vect, fun = NULL, na.rm = TRUE)
slap_means_per_poly <- raw_slap %>%
  group_by(ID) %>%
  summarise(slap_mean = median(.data[[names(Tmean_raster)[1]]], na.rm = TRUE), .groups = "drop") %>%
  pull(slap_mean)
toc()
