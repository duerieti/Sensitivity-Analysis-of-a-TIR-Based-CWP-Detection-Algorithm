...
detect_cwp_single <- function(
    ...
    rounded_ras_path,    # path to the pre-rounded version of the TIR raster
  ...
  # ── 1. BUILD SEGMENTS, ZONES & REFERENCE STRIPS ───────────────────────────
  ...
  # load the pre-rounded TIR raster
  r_tir_rounded <- terra::rast(rounded_ras_path)
  ...
  # ── 4. RECLASSIFY ZONES TO REFERENCE TEMPERATURES ─────────────────────────
  ...
  # ── 10. POLYGON-LEVEL REFILTER ────────────────────────────────────────────
  # The pixel flagging in step 6 operates cell-by-cell, but the polygonisation
  # and morphological closing in step 7 can merge patches whose aggregate
