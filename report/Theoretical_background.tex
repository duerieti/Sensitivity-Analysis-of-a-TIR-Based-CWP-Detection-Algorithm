...
\subsection{Model - Working Definition}
...
\subsection{Model Input Sensitivity Analysis}
...
\subsubsection{The Morris Method}
...
The Morris method occupies itself with finding the statistical distribution of each of the $i$ elementary effects. $\mathcal{F}_i$ is thereby used to denote the distribution of $EE_i$, in other words $\mathcal{F}_i \sim EE_i$. $\mathcal{F}_i$ is a finite discrete distribution, and its number of elements depends on $p$, which is however not that much of interest in an applied setting.
