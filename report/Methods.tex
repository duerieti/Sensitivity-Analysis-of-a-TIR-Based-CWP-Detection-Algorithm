...
\subsection{Optimizationn of Existing Algorithm}

% Existing algorithm has the following runtime
% This is rather long. For sensitivity analysis the algorithm has to be executed 
% 200 is times.
% With the current runtime, this is hindring, especially since setting up computations involves debuging steps and revisions. These steps are then hard to execute given the long runtimes.
% Therefore optimizing the algorithm for faster runtimes is a necessity.
% The current setup relies on a series of computations executed using terra aswell as custom functions for polygonisation of cold water flagged pixels.
% an effort was taken to reexpress large parts of the algorithm as a sequence of geoprocessing opperations that are supported by GDAL. This because executing processing steps using the GDAL CLI is often faster than relying on terra in R. 
% The floowing pseudo code describes the workings of the new algorithm
% 



\subsection{Sensitivity Analysis using Morris}
...
\subsubsection{Morris Experiment for pixel buffer and step length parameters}
...
\subsubsection{Compute morris scores for pixel buffer, step length and temperature delta}
