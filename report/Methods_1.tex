...
The original algorithm focuses on per-slab processing of the raster using a set of R loops and relying on \texttt{terra} and \texttt{sf} functions to implement the logic. The reworked algorithm adapts much of the original logic but instead operates directly on disk: The majority of processing steps read their input from, and writes their output to, raster or vector files via calls to the GDAL command line interface. Table~\ref{tab:cwp_pseudocode} gives a high-level overview of the reimplemented algorithm; the tool or function used in each step is listed in the right-hand column. The remainder of this section motivates the key design decisions underlying this reimplementation.
